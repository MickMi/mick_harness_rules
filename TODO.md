...
- [ ] 修复 `scripts/vibe-init.sh` 幂等性 (防止重复运行覆盖已有内容)
...
| **P3** | 改造 `scripts/vibe-init.sh` | 从"内联所有内容"改为"从 harness 仓库加载" + 集成 `brain mount` | ✅ |
| **P3** | Daily/Weekly compound 蒸馏脚本 | Session → Project → Global 的自动蒸馏流程 | ✅ |
| **P4** | 多平台适配 | Cursor 自动写入规则、多 IDE 注入、brain-rules-template.md | ✅ |
| **P4** | 记忆容量治理 | Session 层 90 天归档、MEMORY.md 最大长度控制、scripts/brain-gc.sh | ✅ |
...
- [x] 同步更新 `scripts/vibe-init.sh` 脚本
