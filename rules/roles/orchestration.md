...
```
━━━━━━ 回合卡片 · <feature名> ━━━━━━
✅ 本回合：<一句话：具体做了什么、结果、关键判断/推理深度>
📍 整体：<状态 — plan X/Y · STATE.md 当前阶段 · 归属谁；无 plan 时写当前状态>
➡️ 下一步：<给用户一个明确动作；需要切工具/角色时给可直接复制的指令>
🧠 状态：<上下文负载约 X-Y%（结构来源）或上下文负载 X%（分类分布）>
🚧 阻塞：<无 / 缺什么 / 谁处理>
```

**各角色 `➡️` 下一步和 `🚧` 阻塞线的标准走向**（见 `extended.md` §10.8 状态机）：

| 当前角色 | 正常完成 → `➡️` 指向 | 卡住 → `🚧 阻塞` 指向 |
|---|---|---|
