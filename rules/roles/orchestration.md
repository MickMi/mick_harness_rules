...
> **可观测性（v4 新增）**
>
> 启用角色编排 / `docs/STATE.md` 工作流时，每次工作流回复的**第一行**必须输出脚手架状态行（Scaffold Status Line），让用户一眼判断 AI 有没有走脚手架。纯解释、短问答、头脑风暴不强制输出状态行。
...
3. **输出 Scaffold Status Line**（v4 新增，回复第一行）→ 格式：`[🎭 {角色} · {feature}@{阶段} · {strictness}/{design}/{testing}]`
   ...
   - 若 `plan.md` 存在且含未完成步骤，`core.md` 铁律 0 的 `[📋 Executor · plan ...]` 首行优先；不要再额外输出第二个首行状态
...
3. 输出 Scaffold Status Line（回复第一行，格式见本文件"可观测性 v4"章节；若 `plan.md` 活跃，则使用 `core.md` 铁律 0 的 plan 首行）
4. 比对用户消息意图：
