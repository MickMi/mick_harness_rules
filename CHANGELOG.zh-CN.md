...
### 个性化 PRD、WorkBuddy 接入与多 Worktree 统一进度

- `prd-for-humans` 成为支持 Agent 的托管 Skill。需求明确时直接输出面向人类评审的 PRD；
  信息不足时只追问会改变范围、产品行为或验收判断的少量问题。
- PRD 的项目事实、个人写作偏好和可选 Brain 记忆具有明确优先级与证据边界。未经审批的
  日志、推断和一次性修订不会静默变成产品事实或个人风格规则。
- 支持检测 WorkBuddy 及其已验证的个人 Skill 目录。工作台分别展示发现、Skill 安装、
  规则加载、执行证据、回写和 Hook 支持，不再根据文件或进程推断运行时已经生效。
- 同一 Git 仓库的多个 Worktree 统一归入一个项目进度源。工作台分别展示已发布标签、
  活跃版本分支和登记主目录，并在结构化事件中保留来源 Worktree、分支与提交。
- PRD 协作支持局部修订和从已认可样例中学习，采用灵活的人类产品文档结构、图文场景表格、
  本地配图检查，并防止过期范围或技术交付指令混入正文。
- 开发验收固定使用 `127.0.0.1:6426`，与正式 `127.0.0.1:6425` 分离。开发环境只读真实
  进度，不采集事件或写入记录；两个环境均显示实际运行版本、来源分支、提交及启动时间。

兼容性：现有项目、Brain 数据、运行账本、Agent Loader 和唯一的 `127.0.0.1:6425`
服务保持兼容。Worktree 聚合属于新增只读能力，不会自动 pull、merge、stash 或改写工作区。

迁移说明：运行 `harness update`。现有更新路径会刷新登记项目与支持的 Agent Skill，
无需迁移项目或 Brain 数据。

验证：265 个 Python unittest、生成规则一致性、两个工作台聚焦交互检查、公开发布审计、
上下文预算、Shell 语法、发布差异检查与全新项目 Quick 安装冒烟均通过。

## [0.24.0] - 2026-09-15

### 快速、可操作的本机项目指挥台

- 工作台先读取轻量项目摘要。项目首页只有在用户进入对应页面时，才继续读取 Git、产物、
  事件历史、Brain、Agent 与 Skill 诊断，移除过去必须等待全部数据才能使用的启动方式。
- 每条注意事项都会说明影响、责任方和下一步。项目阻塞区分为需要用户裁决、Agent 可处理
  或外部环境问题，并提供证据、可复制指令和受控处理路径，不会静默修改项目状态。
- 项目导航更紧凑，支持检索，并按需要关注、正在推进和其他项目分组。待测试、待同步等机器
  状态不再冒充用户任务。
- Harness 改进以一条可追踪链路展示项目证据、模式归纳、人工审批、实现版本和效果复验。
  确定性问题继续优先落为脚本、测试或 Checker，而不是继续增加 Prompt 约束。

兼容性：现有项目、运行账本、Brain 数据、Agent Loader、操作记录和唯一的
`127.0.0.1:6425` 服务保持兼容。摘要响应属于新增能力，原有完整工作区仍可按需读取。

迁移说明：运行 `harness update`。现有更新路径会刷新登记项目和支持的 Agent Loader，
无需迁移项目或 Brain 数据。

验证：244 个 Python unittest、工作台聚焦交互合同、生成规则一致性、Shell 语法、公开发布
审计、上下文预算与非交互全新项目安装冒烟测试均通过。

## [0.23.0] - 2026-09-14

### 项目优先工作台与可追踪 Harness 状态

- 工作台以可执行的项目进度为主体。维护操作收进“管理”，成功结果压缩成一行；只有阻塞、
  审批、同步决策和明确等待用户决策的事项才进入“需要你处理”。
- 操作状态无需刷新页面即可自动更新：执行中快速查询，空闲页面仍能发现其他标签发起的
  操作，断线时按有上限的退避自动重连，并且不会重复执行写操作。
- 每个已接入项目显示实际 Harness 版本及其与本机安装基准的一致性。详情分别说明版本、
  提交、规则内容、加载入口一致性，并明确当前 AI 会话是否加载仍未验证。
- 项目主页先展示长期目标，再展示当前版本；当前需求默认保持紧凑，选择后才展开角色、
  门禁、测试证据和交付记录。文件路径、Git 活动和执行诊断默认收起。

兼容性：现有项目、加载器、Agent Hook、Brain 配置、事件账本和唯一的
`127.0.0.1:6425` 服务保持兼容。新增的项目版本接口为只读，不会声称运行中的 AI 会话
已经重新加载规则。

迁移说明：运行 `harness update`。现有更新路径会刷新登记项目和支持的 Agent 加载器，
无需迁移项目数据。

验证：243 个 Python unittest、工作台聚焦交互合同、生成规则一致性、Shell 语法、公开发布
审计、上下文预算与非交互全新项目安装冒烟测试均通过。
