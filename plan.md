...
- [x] 230. [性能] 拆开全局首屏、项目工作区和诊断数据的串行等待，建立轻量摘要、按需详情和可失效缓存；记录真实 6425 基线并以本地预览对比。
- [x] 231. [阻塞闭环] 将阻塞按用户裁决、Agent 可处理和外部环境分类，为每条阻塞提供来源、影响、责任方、建议动作、证据入口和安全处理路径；不允许界面静默改写项目事实。
- [x] 232. [紧凑导航] 降低侧栏字体、宽度和重复状态噪音，按注意力分组并增加检索；保留项目与子视图层级，不引入分享状态。
- [x] 233. [行动队列] 把阻塞、待裁决、待审批和失败操作统一为可进入处理位置的行动项；待测试、待同步等机器状态不冒充用户待办。
- [x] 234. [Harness 改进] 将项目问题与跨项目模式、改进提案、审批、实现、发布版本和效果复验串成单一生命周期；确定性问题优先指向脚本、测试或 Checker，而不是继续增加 Prompt。
- [x] 235. [预览门禁] 增加聚焦合同测试和性能对比，在隔离端口验证核心点击与窄屏布局；先交体验预览，确认后才运行完整发布回归。
- scope-decision: 用户明确排除上一轮 P1 的分享能力；本版本不实现分享链接、分享快照或跨用户路由状态。
- preservation: 只读诊断仍不得自动修改 `plan.md`、版本状态或阻塞事实；任何解决动作必须展示影响、保留审计并由项目上下文执行。
- baseline: 真实 6425 全局接口约 7–12 秒，项目 workspace 约 8 秒且约 435 KB；当前页面先等待八个全局接口，再读取项目与运行详情，后台扫描与前台读取存在资源竞争。
- authorization: 用户已确认 6432 体验方向，并要求先更新 v0.24.0、再进入 v0.25.0。
- release-contract: v0.24.0 为向后兼容的新增能力版本；现有项目无需迁移数据，发布后由 `harness update` 刷新登记项目、Agent Loader 与唯一 6425 服务。

### v0.24.0 · 2026-09-15 · 第一阶段可体验预览

- implementation: 全局项目摘要、项目概览与完整诊断已拆分；工作台先展示摘要，Git、产物、事件、Brain、Agent 与 Skill 只在相关页面读取。
- implementation: 行动队列已展示影响、责任方与处理入口；项目阻塞已按用户裁决、Agent 可处理、外部环境分类，并提供证据、复制指令和受控 Agent 入口。
- implementation: 左侧项目导航已压缩并按“需要关注 / 正在推进 / 其他项目”分组，支持项目名检索。
- implementation: Harness 改进页已展示“问题证据 → 模式归纳 → 人工审批 → 改进提案 → 实现落地 → 发布版本 → 效果复验”，实现登记补充必填发布版本。
- performance: 隔离预览实测 `/api/portfolio.json` 0.318 秒 / 23,645 B；项目 overview 0.118 秒 / 3,304 B；同项目完整 workspace 0.505 秒 / 8,275 B，后者不再阻塞默认项目主页。
- verification: `node tests/test_operation_feedback.cjs` 通过；8 项聚焦 Python 合同测试通过；HTTP 摘要与按需详情路由测试通过；浏览器真实点击通过工作台、项目阻塞、版本详情和 Harness 改进页。
- preview: `http://127.0.0.1:6432/?view=workbench`；未部署 6425、未提交、未发布，等待用户确认交互方向后再进入 QA。

### Step 235 — 2026-09-15 · v0.24.0 发布门禁
...
- [x] 229. [发布] 完成生成一致性、全量回归、公开内容审计和安装烟测；经 GitHub PR 合并并发布 v0.23.0，更新本机 Harness 与登记项目，最后在唯一的 6425 服务核验版本、健康状态和真实页面。
- authorization: 用户已确认 6431 预览方向，并明确要求“更新一下发布上去”。
- preservation: 使用隔离开发工作区和 PR，不覆盖主检出的 `docs/VERSIONS.md` 本地改动，不处理其未跟踪项目目录；安装更新若发现未说明的本地改动则停止，不自动 stash、reset 或覆盖。

### Step 229 — 2026-09-14 · v0.23.0 发布门禁

- candidate-verify: `python3 scripts/harness-verify.py release` → 243 tests / OK，生成一致性与 `git diff --check` 通过；`node tests/test_operation_feedback.cjs` → PASS；Shell 语法、公开内容审计、上下文预算通过；当前发布树复制到临时新项目后，`setup.sh --non-interactive` → exit 0。
- release-contract: v0.23.0 为向后兼容的新能力版本；现有项目无需数据迁移，发布后通过既有 `harness update` 刷新登记项目、Agent 加载器与唯一 6425 服务。
- remaining-at-commit: GitHub PR、annotated tag、本机更新与 6425 真实页面检查属于本次获授权发布事务；任一步失败即停止并保留真实状态，不把候选验证冒充线上发布。

## v0.23.0 · 2026-09-14 · 项目目标前置，聚焦当前需求

- [x] 228. [修改] `web/observe-dashboard.html`：长期目标置顶，当前版本紧随其后；空计划不显示大块零进度。需求以紧凑摘要展示，点击展开角色与验证详情；活动和执行诊断默认折叠，真实阻塞与待决策保持可见。
- boundary: 仅重排与压缩项目概览，保留真实数据、角色形象、版本标签和现有交互；不修订项目计划/门禁、不部署 6425、不发布。补充聚焦自动检查后交 6431 预览。
- baseline: `node tests/test_operation_feedback.cjs` → PASS / exit 0；6431 只读预览服务可用。

### Step 228 — 2026-09-14 · 项目概览信息密度预览

- implemented: 长期目标成为项目标题后的第一层信息；当前版本压缩为目标、进度和四项统计。需求默认只展示标题、状态、当前角色/下一步，点击后才展开任务小队、测试证据和角色办公室。无计划项目不再渲染零进度、空需求和“无需处理”大卡；活动、路径及执行透明度统一收进默认关闭的“活动与诊断”。
- verify: `node tests/test_operation_feedback.cjs` → PASS / exit 0，覆盖目标排序、空计划减法、用户决策可见、需求展开与二次点击收起；`git diff --check` → exit 0。
- browser: 6431 真实读取一个尚未规划的登记项目与一个包含三条需求的项目。空计划项目的 DOM 主序为标题、长期目标、当前版本、诊断，只保留一个明确下一步；三条需求默认均未展开，实点其中一条后角色与验证详情出现，再点收起；诊断实点可见项目目录、目标来源和执行记录。1440px 桌面视图检查通过，390px 文档宽度等于视口宽度，测试后恢复默认视口和诊断收起状态。
- remaining: 等用户体验确认；未跑全仓回归、未部署 6425、未提交或发布。现有版本状态矛盾仍按真实数据展示，未在本轮篡改为视觉上的完成。

## v0.23.0 · 2026-09-13 · 项目优先，收起维护操作

- [x] 227. [修改] `web/observe-dashboard.html`：按用户选择 A，项目列表为主体；顶部仅展示有明确处理入口的提醒，管理操作折叠，操作运行/失败仍可见，成功缩为一行；减少分割线，统一按钮与状态的视觉区别。保留已认可版本标签，详情改称“规则文件位置”。
- boundary: 复用现有操作、审批、项目跳转与自动查询，不修改后台状态或审批规则；扩充聚焦测试，只交 6431 只读预览，不部署 6425。
- baseline: `node tests/test_operation_feedback.cjs` → PASS / exit 0。

### Step 227 — 2026-09-13 · 项目优先的轻工作台预览

- implemented: 管理操作默认收起，操作结果独立保留且成功仅一行；项目列表为主，顶部仅保留有处理入口的阻塞、用户裁决、审批等提醒，不把待测试数当成人工待办。历史与系统信息默认折叠，版本详情将“接入来源”改为“规则文件位置”。
- verify: `node tests/test_operation_feedback.cjs` → PASS / exit 0，覆盖管理开关、成功单行、正常时隐藏手动查询、断线显示查询、待测试不制造人工提醒；`git diff --check` → exit 0。
- browser: 6431 读取真实数据，管理展开/收起与版本弹窗已实点；关闭管理后状态条仍可见。1440px 桌面截图检查项目列对齐；689px 与 390px 的文档宽度均等于视口宽度，无页面横向溢出，测试后恢复窗口。全页截图存在拼接重复，未据此宣称全页像素验收。
- remaining: 仅可体验预览，等待用户确认；未执行真实更新、未跑全仓回归、未部署 6425、未提交或发布。开发副本未注入，不冒充其他项目写工作事件。

## v0.23.0 · 2026-09-12 · 自动跟踪更新与项目接入版本

- [x] 226. [修改] `web/observe-dashboard.html` 与 `scripts/harness-observe.py`：复用操作查询，增加空闲查询、断线退避重连、恢复后的局部刷新；在项目名称下显示实际 Harness 版本及与本机安装基准的一致性，详情注明提交、规则入口及会话加载未验证。
- boundary: 仅做可体验预览，新增针对性自动检查；不发布、不更新项目规则、不重启 6425、不重复提交更新操作。历史版本不补造 Harness 记录。
- reuse: 复用 `run_git`、项目登记表、操作账本、现有弹窗与样式；单独的只读版本摘要避免高频扫描完整项目进度。
- baseline: `node tests/test_operation_feedback.cjs` → PASS / exit 0。

### Step 226 — 2026-09-12 · 自动跟踪与版本状态预览

- files: `web/observe-dashboard.html`、`scripts/harness-observe.py`、`tests/test_operation_feedback.cjs`、`tests/test_harness_versions.py`。
- implemented: 单请求轮询（执行中 1 秒、空闲 10 秒、断线退避最多 30 秒、后台 60 秒），前台恢复立即查询；仅局部更新状态和版本标签，不重绘表单或重试写操作。只读版本接口按真实挂载、提交、规则摘要与加载入口比较本机安装基准，按根目录复用检查；不把业务仓库 HEAD、版本号相同或未知会话加载当作已生效。
- verify: `node tests/test_operation_feedback.cjs` → PASS / exit 0，覆盖自动断线恢复、空闲发现、后台节流、并发查询合并、版本查询失败保留未知；`python3 -B -m unittest tests.test_harness_versions -q` → 7 tests / OK / exit 0；`git diff --check` → exit 0。
- browser: 6431 只读预览读取真实数据，10 项目均与安装基准 0.22.1 / 2fd504b 一致；点击版本详情展示真实提交与来源，390px 页面 scrollWidth=390。暂停并重启本轮只读代理，同一页面自动从“正在重新连接”恢复“已完成”，未刷新、未触发真实更新；随后将重连后的版本查询改为立即刷新，避免继续等待空闲周期。
- remaining: 等用户体验确认；未跑全仓回归、未发布、未部署 6425、未更新项目规则，未执行真实更新作业。预览代理仅 GET，新 API 读取项目文件不产生回写；操作开始回写因开发副本未注入而 exit 64，未为此初始化或冒充其他项目。
