...
- Branch: main
- Tag: v0.24.0
- Work Branches: feat/v0.24-fast-action-workbench
- Goal: 把 6425 从缓慢的状态观察器升级为快速、可操作的本机项目指挥台，让用户直接处理跨项目待办、进入阻塞解决路径，并把反复出现的问题持续转化为可验证的 Harness 改进。

### Requirements

- [x] `task-230` 重构工作台读取链路：首屏与项目切换不再等待全部全局重查询，重数据按需加载并复用可失效缓存
- [x] `task-231` 建立可操作的阻塞中心：区分用户裁决、Agent 可处理和外部环境阻塞，并提供继续任务、复制指令、查看证据与受控解决入口
- [x] `task-232` 压缩左侧项目导航：降低字号和视觉重量，按需要关注、进行中与其他项目分组，并提供快速检索
- [x] `task-233` 建立统一行动队列：每条提醒必须呈现影响、责任方、建议动作和可进入的处理位置，不再只汇总数量
- [x] `task-234` 将 Harness 改进整理为“问题 → 模式 → 提案 → 审批 → 实现 → 发布版本 → 效果验证”的可追踪闭环，并优先把确定性问题落成脚本或检查器
- [x] `task-235` 完成性能基线、聚焦交互检查和真实本机预览；体验确认后再进入完整回归与发布流程
...
- [x] `task-236` 将 `prd-for-humans` 纳入受支持 Agent 的托管 Skill 与冲突保护
- [x] `task-237` 建立信息充分直出、信息不足多轮收敛且不重复提问的 PRD 对话协议
- [x] `task-238` 固定项目事实、个人 Profile、Brain 偏好与未确认推断的记忆边界
- [x] `task-239` 增加清晰/模糊需求、个人风格生效和范围防幻觉的对话样例与行为测试
- [x] `task-240` 接入 WorkBuddy 的应用检测和已验证个人 Skill 目录，未证实的 Loader 与 Hook 保持未支持
- [ ] `task-241` 在工作台展示四层接入状态，完成聚焦验证和真实预览后再进入发布流程
