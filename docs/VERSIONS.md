...
- Goal: 让用户通过一套跨 Agent 的统一入口建立计划和长期目标、配置 Brain、驱动单条需求端到端交付，同时显著降低 Harness 的日常上下文成本。

### Requirements

- [ ] `task-197` 建立跨 Agent 的 Harness 命令合同：底层统一为稳定 CLI，上层按 Codex、Claude Code 等宿主能力提供薄 Skill 或命令适配；不覆盖 Codex 已有的 `/plan` 与 `/goal`
- [ ] `task-198` 提供计划与目标入口：扫描项目事实、预览冲突后创建或更新 `plan.md`，并把稳定长期目标写入项目目标文件而不是当前版本或技术计划
- [ ] `task-199` 提供 Brain 首次配置和日常管理入口，明确区分“仅本机 / 私有远端 / 暂不启用”，展示写入位置、同步范围和真实连接状态
- [ ] `task-200` 提供绑定单一需求的端到端交付入口，按产品审查、开发、QA 和发布准备门禁推进；权限、歧义、破坏性操作和正式发布仍需用户裁决
- [ ] `task-201` 压缩常驻 Loader，把详细角色与工作流迁移到按需 Skill 和确定性脚本，为全局与项目指令建立可测试的 Token / 字节预算
- [ ] `task-202` 在工作台展示命令能力、执行预览、当前阶段和结果，并完成跨 Agent 兼容、Brain 降级、上下文预算及真实用户路径验证
