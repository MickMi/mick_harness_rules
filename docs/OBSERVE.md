...
工作台把“用户登记的项目”作为稳定身份，但不会假设登记目录一定就是代码仓库：

- 登记目录本身是 Git 仓库时，直接读取它的分支、提交和工作区状态。
- 登记目录下只有一个浅层子 Git 仓库时，自动把这个子仓库作为代码工作区，事件和页面仍归入原项目。
- 同一项目的 ChatGPT/Codex 本地镜像会按镜像中声明的项目标题做唯一匹配；匹配成功后，Agent 回合归入登记项目，不再生成一个重复项目。
- 发现多个同名项目或多个子仓库时停止自动关联并提示用户选择，避免把进度写错项目。

Git 提交和 Agent 回合属于“检测到的真实活动”，可以证明项目已经发生工作，但不会被自动伪造成版本、需求、PM 决策或完成状态。项目尚未建立结构化版本计划时，首页会同时展示真实活动和“尚未规划”，由 PM 再决定如何归档。

执行透明度只展示 Harness 能结构化证明的字段：执行模式、选择或升级原因、工作轮次耗时、Agent 回合和 Harness 命令。Code Agent 宿主没有提供通用工具调用计数时，工作台明确显示“未记录”，不会从聊天文本推测。

## v0.24.0 · 2026-09-15 · 轻量摘要与按需详情

工作台启动时只读取项目的轻量运行摘要，不再串行等待 Git、产物、事件、Brain、Agent 和 Skill 全部扫描完成。项目首页使用 `GET /api/projects/<project_id>/workspace.json?scope=overview`；版本、产物、角色、时间线和事件页仍按需读取完整工作区。`GET /api/portfolio.json?detail=full` 保留完整聚合，默认 `GET /api/portfolio.json` 返回轻量摘要。

摘要和完整详情来自同一份 append-only 运行账本，但用途不同：摘要只服务首屏决策，完整详情服务诊断。页面不会因为后台详情尚未完成而冻结项目导航，也不会把延迟读取显示为数据不存在。

## v0.24.0 · 2026-09-15 · 行动队列、阻塞处理与改进闭环

全局行动队列只收纳用户或系统能够实际进入处理位置的事项。每条记录显示影响、责任方和下一步；待测试、待同步等机器状态只有在存在明确处理入口时才进入用户注意区。

项目阻塞按说明和结构化证据分为需要用户裁决、Agent 可处理、外部环境三类。工作台可以跳到关联需求、展开诊断、复制带项目与验证要求的处理指令，或为非裁决阻塞启动受控 Harness 操作；它不会直接把 `plan.md`、需求状态或阻塞事实改成“已解决”。

Harness 改进页把项目问题串成“问题证据 → 模式归纳 → 人工审批 → 改进提案 → 实现落地 → 发布版本 → 效果复验”。登记实现时必须提供项目相对产物和语义版本；只有后续真实频次下降，才会显示复验有效。

## v0.23.0 · 2026-09-14 · 项目优先工作台与自动状态

工作台首页把项目进展作为主体：顶部只保留有明确处理入口的阻塞、审批、同步决策和用户裁决；维护操作收进“管理”，已完成操作压缩为一行，最近交付与系统状态默认折叠。普通状态文本不伪装成按钮，可点击入口使用一致的链接或按钮反馈。

操作状态由同一个只读查询自动刷新：运行中约每秒查询，空闲页面每 10 秒发现其他标签发起的操作，页面进入后台后降为 60 秒；断线使用最多 30 秒的有界退避，恢复到前台时立即查询。轮询只读取状态，不会重放确认单、重复更新或自动处理失败。

项目列表显示该项目实际挂载的 Harness 版本，以及它与本机安装基准是否一致。详情分别比较版本文件、Harness 提交、规则内容摘要和受管理加载入口；它不会把业务仓库 HEAD、远端最新版本、服务重启或当前 AI 会话已经重新加载规则当成已验证事实。新增接口 `GET /api/harness/versions.json` 只读，不写入项目或 Brain。

## v0.23.0 · 2026-09-14 · 项目主页渐进展示

项目主页先展示长期目标，再展示当前版本及其真实进度。当前需求默认只保留标题、状态、当前角色和下一步；用户选择一条需求后，才展开任务小队、门禁、测试证据、交付历史和对应角色办公室，再次选择即可收起。

项目路径、Git 活动、检测到的回合与执行透明度统一进入默认关闭的“活动与诊断”。真实阻塞与需要用户裁决的事项仍保持可见；尚未建立版本计划的项目不再渲染巨大的零进度和空需求卡，只给出 PM 需要完成的下一步。
