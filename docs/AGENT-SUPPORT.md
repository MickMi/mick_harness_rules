...
| WorkBuddy | Tier 2 | 是 | 项目规则与个人 Skill | 待真实会话 | 否 | 5.3.14 会读取项目根 `AGENTS.md`，个人 Skill 目录为 `~/.workbuddy/skills/`；未发现可验证生命周期 Hook |
...
当前边界：Claude Code 与 Codex 为完整 `managed`；WorkBuddy 的项目规则入口与个人 Skill 目录已有本机宿主证据，因此 Skill 可托管，但生命周期 Hook 仍为 `unsupported`；Cursor、Windsurf、Trae 为 `manual`；Cline 与 Roo 为 `unsupported`。文件存在仍不等于本次会话已加载，加载证明继续只来自真实运行事件。
...
当前扫描目录为 Harness `rules/skills/`、`~/.codex/skills/`、`~/.claude/skills/`、`~/.workbuddy/skills/`、`~/.agents/skills/`、Codex 插件缓存中的 Skills，以及已登记项目的 `.harness/skills/`。扫描器不接受前台提供的任意路径，不读取凭据，不返回 Skill 正文，不运行 `scripts/`，也不联网安装。

v0.25 的受管 Skill 包含 `harness-plan`、`harness-goal`、`harness-brain`、`harness-e2e` 与按需触发的 `prd-for-humans`。`harness agents sync` 会为 Codex、Claude Code 建立链接；仅在检测到 WorkBuddy 后才写入 `~/.workbuddy/skills/`。若用户已有同名文件或目录则报告冲突并保留原内容。链接存在仍只代表“已安装”；WorkBuddy 没有生命周期 Hook，因此必须通过真实会话行为另行验证加载。
